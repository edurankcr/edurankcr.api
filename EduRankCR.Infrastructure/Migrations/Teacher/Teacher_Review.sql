...
CREATE INDEX IX_TeacherReviews_UserId ON Teacher_Reviews(UserId);
CREATE INDEX IX_TeacherReviews_TeacherId ON Teacher_Reviews(TeacherId);
CREATE INDEX IX_TeacherReviews_InstituteId ON Teacher_Reviews(InstituteId);

-- Index for filtering by Status (e.g., approved/pending/deleted reviews)
CREATE INDEX IX_TeacherReviews_Status ON Teacher_Reviews(Status);

-- Index for filtering by CourseMode (e.g., online/in-person/hybrid courses)
CREATE INDEX IX_TeacherReviews_CourseMode ON Teacher_Reviews(CourseMode);

-- Composite Index for filtering by TeacherId and Status (commonly used together)
CREATE INDEX IX_TeacherReviews_Teacher_Status ON Teacher_Reviews(TeacherId, Status);

-- Composite Index for filtering by InstituteId and Status (common filter for institution-based searches)
CREATE INDEX IX_TeacherReviews_Institute_Status ON Teacher_Reviews(InstituteId, Status);

-- Composite Index for filtering by FreeCourse and CourseMode (useful for searching courses)
CREATE INDEX IX_TeacherReviews_FreeCourse_CourseMode ON Teacher_Reviews(FreeCourse, CourseMode);

-- Composite Index for filtering by ProfessorRating and DifficultyRating (common filters in review aggregations)
CREATE INDEX IX_TeacherReviews_Professor_Difficulty_Rating ON Teacher_Reviews(ProfessorRating, DifficultyRating);

-- Index for sorting by CreatedAt (e.g., latest reviews first)
CREATE INDEX IX_TeacherReviews_CreatedAt ON Teacher_Reviews(CreatedAt DESC);

-- Composite Index for filtering and sorting by TeacherId and CreatedAt
CREATE INDEX IX_TeacherReviews_Teacher_CreatedAt ON Teacher_Reviews(TeacherId, CreatedAt DESC);

-- Composite Index for filtering and sorting by InstituteId and CreatedAt
CREATE INDEX IX_TeacherReviews_Institute_CreatedAt ON Teacher_Reviews(InstituteId, CreatedAt DESC);
...
CREATE INDEX IX_TeacherReviews_Teacher_ProfessorRating_Experience
    ON Teacher_Reviews(TeacherId, ProfessorRating) INCLUDE (ExperienceText, DifficultyRating, Status, CreatedAt);
...
CREATE INDEX IX_TeacherReviews_Institute_ProfessorRating_Experience
    ON Teacher_Reviews(InstituteId, ProfessorRating) INCLUDE (ExperienceText, DifficultyRating, Status, CreatedAt);
